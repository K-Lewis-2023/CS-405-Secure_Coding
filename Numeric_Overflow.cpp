/* 
*  Title:       1-3 Activity: Numeric Overflow Coding
*  Name:        Kelly Lewis
*  Date:        5-4-2023
*  Class:       CS-405 Secure Coding
*  Description: Students were tasked with creating Overflow and Underflow protections with
*               reporting errors if an Overflow or Underflow occurred. Source Code was
*               provided and we were to modify the provided code to prevent an Overflow or
*               Underflow then output a message indicating that it was prevented.
*/
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    // *** Begin Student Code ***
    T max = std::numeric_limits<T>::max(); //Get Max for variable

    T temp = max - increment; //Create temp variable for increment max value

    //Begin Step Loop
    for (unsigned long int i = 0; i < steps; ++i) {
        //Try-Catch for exceptions
        try {
            if (result > temp) { //If result will be greater than max, throw error
                throw std::overflow_error("\nERROR: Overflow detected! Last good value: ");
            }
            if (!(result > temp)) { //If result will NOT be greater than max, continue
                result += increment;
            }
        }
        catch (const std::exception& e) { //Exception handling
            std::cerr << e.what(); //error report
            break; //break loop
        }
    }
    // *** End Student Code ***
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    // *** Begin Student Code ***
    T min = std::numeric_limits<T>::min(); //Get lowest for variable

    T temp = min + decrement; //Create temp variable for decrement lowest value

    //Begin Step Loop
    for (unsigned long int i = 0; i < steps; ++i) {
        //Try-Catch for exceptions
        try {
            if (result < temp) { //If result will be less than the lowest value, throw error
                throw std::underflow_error("\nERROR: Underflow detected! Last good value: ");
            }
            if (!(result < temp)) { //If result will NOT be less than lowest value, continue
                result -= decrement;
            }
        }
        catch (const std::exception& e) { //Exception handling
