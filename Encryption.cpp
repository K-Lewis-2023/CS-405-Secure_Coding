...
{
  std::ifstream file_path(filename); //Input
  std::string file_text; //Output

  //Implement loading the file into a string
  try {
      // File open success
      if (file_path) {
          // Create new stream to buffer file content to
          std::ostringstream stringStream;

          // Read buffer to string stream buffer
          stringStream << file_path.rdbuf();

          // Output buffer to file_text
  ...
  catch (...) { //Catch exceptions pertaining to file read
      std::cout << "Read File Failed: Unknown exception..." << std::endl;
  }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //Time Stamp Object / Buffer with local time
  struct tm buf;
  auto t = std::time(nullptr);
  auto tm = localtime_s(&buf, &t);

  //Output stream of file
  ...
  try {
      ...
      std::cout << "Write File Failed: Unknown Exception..." << std::endl;
  }
}
