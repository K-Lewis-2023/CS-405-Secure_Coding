...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  /* BEGIN Student Code 1/2 */
  bool isInjection = false; //Flag for injection detection
  std::string SQLContainment = sql; //Containment abstraction

  //SQL injection Flags
  const char SQLflagA = SQLContainment.find("' or");
  

  //Weigh against total length to ensure substring detected
  if (SQLflagA < SQLContainment.length()) {

      isInjection = true; //Raise Flag

      //Cleanse to prevent possible damage
      SQLContainment.erase(SQLflagA, 1);

      //Check for secondary flag
      const char SQLflagB = SQLContainment.find('=');
      if (SQLflagB < SQLContainment.length()) {
          //If detected, cleanse string.
          SQLContainment.erase(SQLflagB, 1); 
      }
  ...
  char* error_message;
  if(sqlite3_exec(db, SQLContainment.c_str(), callback, &records, &error_message) != SQLITE_OK)
  {
    std::cout << "Data failed to be queried from USERS table. ERROR = " << error_message << std::endl;

    /* BEGIN Student Code 2/2 */
    //If Injection is detected, display warning with indication
    if (isInjection) {
        std::cout << "*** Caution! Possible SQL Injection Detected! Indication: \"" << sql.substr(sql.find("'")) << "\" ***\n" << std::endl;
    }
